...
**"Once you start thinking in events, a new question shows up naturally: If writing data and reading data are such different jobs, why do we force one single model to do both?"** {.hero-margin}

Think about a restaurant. In the back, the kitchen is a **write-heavy**, correctness obsessed machine with a lot of moving parts. Orders come in, they get validated (is that ingredient even in stock), they get sequenced, cooked in the right order, plated exactly right. {.marginHalf}

![A restaurant kitchen next to a menu board — the write model and the read model](/images/posts/restraunt.png) {.margin1}
